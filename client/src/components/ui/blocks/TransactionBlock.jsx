...
import {
  generateP2PTransaction, generatePeer, signTransaction, verifyTransaction,
} from '../../../utils/ecdsa';
...
export default function TransactionBlock() {
  ...
  const signHandler = () => {
    ...
    // ).then((sig) => {
    //   setMessage((prev) => ({
    //  ...prev, signature: Buffer.from(sig).toString('hex'), _sig: sig }));
  ...
  useEffect(() => {
    ...
    //   setKeypair(keys);
    //   const privateKey =
    // Buffer.from(await window.crypto.subtle.exportKey('pkcs8', keys.privateKey)).toString('hex');
    //   const publicKey =
    // Buffer.from(await window.crypto.subtle.exportKey('spki', keys.publicKey)).toString('hex');
